...
class RRTPlanner(PathPlanner):
    ...
    """
    def __init__(self, map, iterations=1e6, epsilon=0.05, stepSize=5):
...
#            return Point(self.map.left + self.map.length * random.random(), self.map.down + self.map.width * random.random())
            return Point(self.map.left + self.map.length * random.random(), self.map.top + self.map.width * random.random())
        else:
            return target
...
#                return True
        return self.map.check_collision(pos, 10)
